...
type Scheduler interface {
	...
	TaskOutput(ctx context.Context, task *Task, db Database) (*Task, error)
}
...
func (m *MockScheduler) TaskOutput(ctx context.Context, task *Task, db Database) (*Task, error) {
	sampleOutput := []byte(`
		this is a sample output generated by MockScheduler...
	`)
	task.Output = ioutils.NewReadCloserWrapper(bytes.NewReader(sampleOutput), func() error {
		return nil
	})
